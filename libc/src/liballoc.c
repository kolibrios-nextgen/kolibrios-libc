 ...
 * SPDX-FileCopyrightText: 2013-2014 Durand Miller <clutter@djm.co.za>
...
#define liballoc_lock()           /* TODO: needs to be implemented for thread safety */
#define liballoc_unlock()         /* TODO: needs to be implemented for thread safety */
#define liballoc_alloc(pages)     _ksys_alloc(pages *KSYS_PAGE_SIZE)
#define liballoc_free(ptr, pages) _ksys_free(ptr);
...
static unsigned int l_pageSize = KSYS_PAGE_SIZE; ///< The size of an individual page. Set up in liballoc_init.
...
void *realloc(void *p, size_t size)
{
    ...
    if (min->magic != LIBALLOC_MAGIC) {
        ...
        // Check for overrun errors. For all bytes of LIBALLOC_MAGIC
        if (((min->magic & 0xFFFFFF) == (LIBALLOC_MAGIC & 0xFFFFFF)) || ((min->magic & 0xFFFF) == (LIBALLOC_MAGIC & 0xFFFF)) || ((min->magic & 0xFF) == (LIBALLOC_MAGIC & 0xFF))) {
            l_possibleOverruns += 1;
#if defined LIBALLOC_DEBUG || defined INFO
            printf("liballoc: ERROR: Possible 1-3 byte overrun for magic %x != %x\n",
                min->magic,
